  ...
  return {
    default: defineComponent({
      ...
      setup(props, { expose }) {
        ...
        expose({
          insert: (text: string) => inserted.push({ label: props.label, text }),
        });
...
function build(texts = { intro: '<p>Beste klant</p>', footer: '<p>Voorwaarden</p>' }) {
...
function hiddenValue(wrapper: VueWrapper, name: string) {
  return (wrapper.find(`input[type="hidden"][name="${name}"]`).element as HTMLInputElement).value;
}
...
describe('premade-texts/Edit', () => {
  ...
  it('opens both texts for editing', () => {
    ...
    expect(editorFor(wrapper, 'Intro text').props('modelValue')).toBe('<p>Beste klant</p>');
    expect(editorFor(wrapper, 'Footer text').props('modelValue')).toBe('<p>Voorwaarden</p>');
  });

  it('submits what the editors hold', async () => {
    const wrapper = build();

    editorFor(wrapper, 'Intro text').vm.$emit('update:modelValue', '<p>Herschreven intro</p>');
    editorFor(wrapper, 'Footer text').vm.$emit('update:modelValue', '<p>Herschreven footer</p>');
  ...
  it('keeps the hidden fields in step with the editors', async () => {
    ...
    editorFor(wrapper, 'Footer text').vm.$emit('update:modelValue', '<p>Nieuw</p>');
    await wrapper.vm.$nextTick();
...
describe('the placeholder picker', () => {
  ...
  it('inserts the token rather than the label it shows', async () => {
    ...
    expect(inserted).toEqual([{ label: 'Intro text', text: '[[[customer_first_name]]]' }]);
  });
  ...
  it('sends the token to the text whose button was pressed', async () => {
    ...
    expect(inserted).toEqual([{ label: 'Footer text', text: '[[[customer_company_name]]]' }]);
  });
  ...
  it('does not steal the selection from the editor', () => {
    const chip = build().findAllComponents(PlaceholderPicker)[0].find('button');
