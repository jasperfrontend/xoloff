...
describe('product-categories', () => {
    ...
    it('says so when there are none', () => {
        expect(
            mount(CategoryIndex, { props: { categories: [] } }).text(),
    ...
    it('promises that deleting a category keeps its products', () => {
        ...
        expect(
            wrapper.findComponent(ConfirmDeleteButton).props('description'),
        ).toContain('become uncategorised');
    });
...
describe('tax-classes', () => {
    ...
    it('says so when there are none', () => {
        expect(
            mount(TaxClassIndex, { props: { taxClasses: [] } }).text(),
        ).toContain('No tax classes yet');
    });
    ...
    it('stays quiet when the last delete did not fail', () => {
        ...
        expect(wrapper.find('[class*="bg-destructive/10"]').exists()).toBe(
            false,
        );
    });
    ...
    it('edits starting from the current rate', () => {
        ...
        const percentage = wrapper.find('input[name="percentage"]')
            .element as HTMLInputElement;
