...
watch(
  () => [form.discount_type, form.discount_value, form.rounding_override, form.line_items],
  refreshTotals,
  { deep: true },
);
...
function applyProduct(lineItem: LineItemState, productId: string) {
  const product = props.products.find((candidate) => candidate.id === Number(productId));
  ...
  lineItem.specs = product.specs.length
    ? Object.fromEntries(product.specs.map((spec) => [spec.key, spec.value]))
    : null;
}
...
function submit() {
  form
    .transform(() => ({ customer_id: form.customer_id, ...toPayload() }))
    ...
    <form class="space-y-6" @submit.prevent="submit">
      <fieldset :disabled="readOnly" class="space-y-6">
        ...
        <div class="grid gap-3 rounded-xl border p-4">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-xs text-foreground">
                Pick a product to fill the line, then edit anything you like. Catalog values are
                only defaults.
              </p>
            </div>
            <Button
          ...
          <p v-if="form.line_items.length === 0" class="text-sm text-foreground">
          ...
          >
            <div class="flex items-start justify-between gap-2">
              <div class="grid flex-1 gap-2">
                <Label :for="`line-${lineIndex}-product`"> Insert from catalog </Label>
                <Select
                  :model-value="lineItem.product_id?.toString()"
                  @update:model-value="applyProduct(lineItem, String($event))"
                >
                  <SelectTrigger :id="`line-${lineIndex}-product`" class="cursor-pointer">
                    <SelectValue placeholder="Start from a blank line" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem
            ...
            <div class="grid gap-2">
              <Label :for="`line-${lineIndex}-name`">Description</Label>
              <Input :id="`line-${lineIndex}-name`" v-model="lineItem.name" required />
              <InputError :message="form.errors[`line_items.${lineIndex}.name`]" />
            </div>

            <div class="grid gap-3 sm:grid-cols-3">
              <div class="grid gap-2">
                <Label :for="`line-${lineIndex}-quantity`">Quantity</Label>
                <Input
                  ...
                  required
                />
                <InputError :message="form.errors[`line_items.${lineIndex}.quantity`]" />
              </div>

              <div class="grid gap-2">
                <Label :for="`line-${lineIndex}-price`">Unit price ex. VAT</Label>
                <Input
                  ...
                  min="0"
                  required
                  @blur="settleLineAmount(lineItem, 'unit_price_ex_vat')"
                />
                <InputError :message="form.errors[`line_items.${lineIndex}.unit_price_ex_vat`]" />
              </div>

              <div class="grid gap-2">
                <Label :for="`line-${lineIndex}-tax`">Tax class</Label>
                <Select
                  :model-value="lineItem.tax_class_id?.toString()"
                  @update:model-value="lineItem.tax_class_id = Number($event)"
                >
                  <SelectTrigger :id="`line-${lineIndex}-tax`" class="cursor-pointer">
                ...
                <InputError :message="form.errors[`line_items.${lineIndex}.tax_class_id`]" />
              </div>
            </div>

            <div class="grid gap-3 sm:grid-cols-2">
              <div class="grid gap-2">
                <Label :for="`line-${lineIndex}-discount-type`">Line discount</Label>
                <Select
                  :model-value="lineItem.discount_type ?? 'none'"
                  @update:model-value="
                    lineItem.discount_type =
                      $event === 'none' ? null : ($event as 'percentage' | 'fixed');
                    clearDiscount(lineItem);
                  "
                >
                  <SelectTrigger :id="`line-${lineIndex}-discount-type`" class="cursor-pointer">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="none" class="cursor-pointer"> No discount </SelectItem>
                    <SelectItem value="percentage" class="cursor-pointer"> Percentage </SelectItem>
                    <SelectItem value="fixed" class="cursor-pointer"> Fixed amount </SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div v-if="lineItem.discount_type" class="grid gap-2">
                <Label :for="`line-${lineIndex}-discount-value`">
                  {{ lineItem.discount_type === 'percentage' ? 'Percentage' : 'Amount' }}
                </Label>
                <Input
                  ...
                  step="0.01"
                  min="0"
                  :max="lineItem.discount_type === 'percentage' ? 100 : undefined"
                  @blur="settleLineAmount(lineItem, 'discount_value')"
                />
                <InputError :message="errorFor(`line_items.${lineIndex}.discount_value`)" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="grid gap-3 rounded-xl border p-4 sm:grid-cols-2">
          <div class="grid gap-2">
            ...
            <Select
              ...
              @update:model-value="
                form.discount_type = $event === 'none' ? null : ($event as 'percentage' | 'fixed');
                clearDiscount(form);
              "
            >
              <SelectTrigger id="discount_type" class="cursor-pointer">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="none" class="cursor-pointer"> No discount </SelectItem>
            ...
            <p class="text-xs text-foreground">
              Applied before VAT and split across the lines in proportion to their value.
            </p>
          </div>
        ...
        <div class="grid max-w-md gap-3 rounded-xl border p-4">
          <div class="grid gap-2">
            ...
            />
            <p class="text-xs text-foreground">
              Replaces the total outright. There is no reconciliation line and the calculated total
        ...
        <div class="flex items-center gap-4">
          ...
          <Button variant="ghost" as-child class="cursor-pointer">
            <Link :href="index()">{{ readOnly ? 'Back' : 'Cancel' }}</Link>
          </Button>
        </div>
      </fieldset>
    </form>
