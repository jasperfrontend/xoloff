...
describe('quotes/Edit', () => {
  ...
  it('allows saving as a new version', () => {
    ...
    expect(wrapper.findComponent(QuoteForm).props('newVersionUrl')).toContain('/versions');
  });
  ...
  it('shows the link and the window once the quote has been sent', () => {
    ...
    const link = wrapper.findAll('a').find((anchor) => anchor.text().includes('/offerte/abc'));

    expect(link?.attributes('href')).toBe('https://xoloff.test/offerte/abc');
    expect(wrapper.text()).toContain('anna@acme.test');
    expect(wrapper.text()).toContain('20-09-2026');
    expect(wrapper.findComponent(SendQuoteDialog).props('alreadySent')).toBe(true);
  });
  ...
  it('offers the quote as a download', () => {
    ...
    const link = wrapper.findAll('a').find((anchor) => anchor.text() === 'Download PDF');
