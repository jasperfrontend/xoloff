...
<template>
  <aside class="sticky top-4 grid gap-3 rounded-xl border p-4">
    <div class="flex items-center justify-between">
      ...
      <span v-if="calculating" class="text-xs text-muted-foreground"> Calculating </span>
    </div>
    ...
    >
      These totals are out of date. Something in the quote is incomplete, so the figures below are
      from the last version that added up.
    </p>
    ...
    <dl v-else class="grid gap-2 text-sm" :class="{ 'opacity-50': stale }">
      <div v-if="totals.quoteDiscount !== '0.00'" class="flex justify-between">
      ...
      <div v-if="totals.quoteDiscount !== '0.00'" class="flex justify-between">
        ...
        <dd class="tabular-nums">- {{ formatMoney(totals.quoteDiscount) }}</dd>
      </div>
      ...
      >
        <dt class="text-foreground">
          ...
          <span class="text-muted-foreground"> over {{ formatMoney(taxClassTotal.net) }} </span>
        </dt>
        <dd class="tabular-nums">
