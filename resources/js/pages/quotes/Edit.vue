  ...
  <div class="flex flex-col space-y-6 p-4">
    <div class="flex items-start justify-between gap-4">
      <div class="flex items-start gap-3">
        ...
        <QuoteStatusBadge :status="quote.status" :label="quote.status_label" />
      </div>
      ...
      <div class="flex items-center gap-2">
        ...
        <Button variant="secondary" as-child>
          ...
          <a :href="QuotePdfController.current(props.quote.id).url"> Download PDF </a>
        </Button>

        <Button v-if="quote.version_count > 1" variant="secondary" as-child>
          <Link :href="QuoteVersionController.index(props.quote.id).url"> Version history </Link>
        </Button>
      </div>
    </div>
    ...
    >
      The customer has
      {{ quote.status === 'approved' ? 'approved' : 'declined' }} this quote, so it is kept exactly
      as they answered it and can no longer be changed or deleted. Raise a new quote to offer
      different terms.
    </p>
    ...
    >
      ...
      <p v-if="quote.deny_reason" class="whitespace-pre-line text-foreground">
        {{ quote.deny_reason }}
      </p>
    ...
    <!--
            ...
            hand.
        -->
    <div v-if="quote.magic_link" class="grid gap-2 rounded-xl border p-4 text-sm">
      <div class="flex flex-wrap items-center gap-x-4 gap-y-1">
        <span class="font-medium">Sent to {{ quote.customer_email }}</span>
    ...
    <QuoteForm
      ...
      :new-version-url="quote.is_editable ? storeVersion(quote.id).url : undefined"
