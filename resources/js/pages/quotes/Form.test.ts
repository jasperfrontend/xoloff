...
describe('quotes/Form', () => {
  ...
  it('fills a line from the catalog without locking it', async () => {
    ...
    expect((description.element as HTMLInputElement).value).toBe('Managed hosting');

    // The line takes the product's own tax class, not the first in the list.
    await description.setValue('Renamed by hand');
    await wrapper.find('form').trigger('submit');

    const lineItems = submissions[0].data.line_items as Record<string, unknown>[];
  ...
  it('asks the server to reprice when the content changes', async () => {
    ...
    const previews = submissions.filter((submission) => submission.url.includes('preview'));
  ...
  it('keeps the rounding override in a card of its own', async () => {
    ...
    const discountCard = wrapper.find('#discount_value').element.closest('.rounded-xl');
    const overrideCard = wrapper.find('#rounding_override').element.closest('.rounded-xl');
