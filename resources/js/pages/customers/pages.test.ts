...
describe('customers/Form', () => {
    ...
    it('demands both halves of the name but not the salutation', () => {
        ...
        expect(
            wrapper.find('input[name="first_name"]').attributes('required'),
        ).toBeDefined();
        expect(
            wrapper.find('input[name="last_name"]').attributes('required'),
        ).toBeDefined();
    });
    ...
    it('offers no salutation as a choice and sends it as null', () => {
        ...
        expect(selectNamed(wrapper, 'salutation')?.props('defaultValue')).toBe(
            'none',
        );
        expect(wrapper.text()).toContain('None');

        const transform = wrapper
            .findComponent({ name: 'FormStub' })
            .props('transform') as (
    ...
    it('keeps the salutation an existing customer already has', () => {
        expect(
            selectNamed(buildForm({ customer }), 'salutation')?.props(
                'defaultValue',
            ),
        ).toBe('heer');
    });
    ...
    it('defaults a new customer to the Netherlands', () => {
        ...
        expect(selectNamed(buildForm(), 'country')?.props('defaultValue')).toBe(
            'NL',
        );
    });
    ...
    it('keeps the country an existing customer already has', () => {
        expect(
            selectNamed(buildForm({ customer }), 'country')?.props(
                'defaultValue',
            ),
        ).toBe('US');
    });
    ...
    it('starts from the customer it is editing', () => {
        ...
        expect(valueOf('textarea[name="billing_address"]')).toBe(
            'Desert Road 1',
        );
    });
});
