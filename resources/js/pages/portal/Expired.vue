...
<template>
  ...
  <div class="rounded-xl border bg-background p-8">
    ...
    <h1 class="mt-6 text-xl font-semibold">Deze offerte is niet meer geldig</h1>

    <p v-if="quote.valid_until" class="mt-2 text-foreground">
      Offerte {{ quote.id }} was geldig tot en met {{ formatDate(quote.valid_until) }}.
    </p>

    <p class="mt-4 text-sm text-foreground">
      Neem gerust contact met ons op als u de offerte alsnog wilt bekijken. We sturen u dan een
      nieuwe.
    </p>
  </div>
</template>
