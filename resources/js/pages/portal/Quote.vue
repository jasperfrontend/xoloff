...
function netFor(lineItem: PortalLineItem): string | null {
    return (
        props.totals?.lines.find((line) => line.lineItemId === lineItem.id)
            ?.net ?? null
    );
}
</script>
            ...
            <template v-if="version && totals">
                <div class="mt-8 overflow-x-auto">
                    <table class="w-full text-sm">
                        ...
                        <tbody>
                            <tr
                                v-if="version.line_items.length === 0"
                                class="border-b"
                            >
                                <td
                                    colspan="5"
                                    class="py-6 text-center text-foreground"
                                >
                            ...
                            >
                                <td class="py-3 pr-4">
                                    ...
                                    <dl
                                        v-if="
                                            specEntries(lineItem.specs).length
                                        "
                                ...
                                <td
                                    class="px-2 py-3 text-right whitespace-nowrap tabular-nums"
                                >
                                ...
                                >
                                    {{
                                        formatPercentage(
                                            lineItem.tax_class_percentage,
                                        )
                                    }}
                                </td>
                                <td
                                    class="py-3 pl-2 text-right whitespace-nowrap tabular-nums"
                                >
                                    {{
                                        netFor(lineItem) === null
                                            ? '-'
                                            : formatMoney(netFor(lineItem)!)
                                    }}
                                </td>
                            </tr>
                        </tbody>
                ...
                <table class="mt-6 ml-auto w-full max-w-sm text-sm">
                    <tbody>
                        ...
                        <tr class="border-t">
                            ...
                            <td
                                class="pt-3 text-right text-base font-semibold tabular-nums"
                            >
