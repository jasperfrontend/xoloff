...
describe('audit-log/Index', () => {
  ...
  it('keeps the detail out of the way until it is asked for', async () => {
    ...
    await wrapper.find('button[aria-label="Details of entry 1"]').trigger('click');
  ...
  it('opens with the filters the server was given', () => {
    ...
    expect((wrapper.find('#from').element as HTMLInputElement).value).toBe('2026-08-01');
    expect(wrapper.findAll('button').find((b) => b.text() === 'Clear filters')).toBeDefined();
  });
  ...
  it('distinguishes an empty log from an empty result', async () => {
    ...
    expect(build({ entries: empty }).text()).toContain('Nothing has happened yet');
