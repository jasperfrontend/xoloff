      ...
      <table class="w-full text-sm">
        ...
        <tbody>
          <tr v-if="categories.length === 0">
            <td colspan="3" class="px-4 py-8 text-center text-foreground">No categories yet.</td>
          </tr>
          <tr v-for="category in categories" :key="category.id" class="border-t">
            <td class="px-4 py-3">
              <Link
                :href="ProductCategoryController.edit(category.id).url"
            ...
            <td class="px-4 py-3 text-right">
              <ConfirmDeleteButton
                :action="ProductCategoryController.destroy.form(category.id)"
