...
const specs = ref<Spec[]>(props.product?.specs?.map((spec) => ({ ...spec })) ?? []);
...
<template>
  <Form v-bind="action" class="max-w-2xl space-y-6" v-slot="{ errors, processing }">
    <div class="grid gap-2">
      <Label for="name">Name</Label>
      <Input id="name" name="name" :default-value="product?.name" required autofocus />
    ...
    <div class="grid gap-2">
      ...
      <Select name="tax_class_id" :default-value="product?.tax_class_id?.toString()">
        ...
        <SelectContent>
          ...
          >
            {{ taxClass.name }} ({{ formatTaxRate(taxClass.percentage) }})
          </SelectItem>
        </SelectContent>
      </Select>
    ...
    <div class="grid gap-2">
      ...
      <Select name="category_id" :default-value="product?.category_id?.toString()">
    ...
    <div class="grid gap-3 rounded-xl border p-4">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-xs text-foreground">
            Billing period, startup cost, contract duration - any key/value pair.
          </p>
        </div>
        <Button type="button" variant="secondary" size="sm" @click="addSpec">
          <Plus class="size-4" />
          Add
        </Button>
      </div>

      <p v-if="specs.length === 0" class="text-sm text-foreground">No specifications yet.</p>

      <div v-for="(spec, specIndex) in specs" :key="specIndex" class="flex items-start gap-2">
        <div class="flex-1">
          <Input
            v-model="spec.key"
