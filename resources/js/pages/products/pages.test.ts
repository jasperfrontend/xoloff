...
describe('products/Index', () => {
  ...
  it('says so when there are none', () => {
    expect(mount(ProductIndex, { props: { products: [] } }).text()).toContain('No products yet');
  });
...
describe('products/Form', () => {
  ...
  it('starts from the product it is editing', () => {
    ...
    const price = wrapper.find('input[name="price_ex_vat"]').element as HTMLInputElement;
  ...
  it('adds and removes spec rows, keeping the indexes contiguous', async () => {
    ...
    await wrapper.findAll('button[aria-label="Remove specification"]')[0].trigger('click');
...
describe('products/Create and Edit', () => {
  it('create offers a blank form', () => {
    ...
    expect(wrapper.findComponent(ProductForm).props('product')).toBeUndefined();
  });
  ...
  it('edit passes the product through and names it', () => {
    ...
    expect(wrapper.text()).toContain('Managed hosting');
    expect(wrapper.findComponent(ProductForm).props('product')).toMatchObject({ id: 7 });
    expect(wrapper.findComponent(ProductForm).props('submitLabel')).toBe('Save changes');
  });
});
