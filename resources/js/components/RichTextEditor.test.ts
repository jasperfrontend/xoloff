...
describe('RichTextEditor', () => {
  ...
  it('names itself for anyone not using a mouse', async () => {
    ...
    expect(wrapper.find('[contenteditable]').attributes('aria-label')).toBe('Intro text');
  });

  it('reports what was typed as html', async () => {
    const wrapper = build();

    await wrapper.find('button[aria-label="Bulleted list"]').trigger('click');
    await settle(wrapper);
  ...
  it('cannot undo before anything has been done', async () => {
    ...
    expect(wrapper.find('button[aria-label="Undo"]').attributes('disabled')).toBeUndefined();
  });
  ...
  it('cannot remove a link where there is none', async () => {
    ...
    expect(wrapper.find('button[aria-label="Remove link"]').attributes('disabled')).toBeDefined();
  });
  ...
  it('inserts text where the caret last was', async () => {
    ...
    expect(latestValue(wrapper)).toBe('<p>Beste [[[customer_first_name]]],</p>');
  });
