...
describe('useCurrentUrl', () => {
  ...
  it('recognises a page below the one it is given', () => {
    expect(isCurrentOrParentUrl('/settings/app', '/settings/app')).toBe(true);
    expect(isCurrentOrParentUrl('/settings/app', '/settings/app/logo')).toBe(true);
  });
  ...
  it('does not treat a shared prefix as a parent', () => {
    expect(isCurrentOrParentUrl('/settings/app', '/settings/appearance')).toBe(false);
  ...
  it('compares an absolute address by its path', () => {
    expect(isCurrentOrParentUrl('http://localhost/settings/app', '/settings/app/logo')).toBe(true);
  });
});
